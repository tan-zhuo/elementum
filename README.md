...
- **分子** —— 20 种常见分子（水、氧气、二氧化碳、甲烷、苯……），点击查看**真实三维分子结构**
...
这套校验抓到过一个真实 bug：过氧化氢的二面角被构造成了补角（68.5° 而非 111.5°），
因为方位角助手在轴反向时会翻转参考系。键长校验发现不了这个问题，角度校验可以。
...
**分子键的绘制**
每条键在中点断开、两半分别用相邻原子的颜色，这样不开标签也能看出组成。双键、
三键的平行线沿构建期算好的垂直方向偏移，该方向取自与相邻键共面的平面，因此苯、
乙烯这类平面分子的多重键会落在分子平面内，而不是垂直翘出去。
...
- 主题为固定深色，未实现浅色主题切换。
- 分子板块目前收录 20 种常见分子。臭氧和苯画的是单一路易斯结构（凯库勒式），
  实际为共振离域，详情页已注明。
